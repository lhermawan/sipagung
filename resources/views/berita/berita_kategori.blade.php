        ...
        <div class="row">
            ...
            <h2 class="word-rotator slide font-weight-bold text-8 mb-0 appear-animation" data-appear-animation="maskUp">
									...
									<span class="word-rotator-words bg-primary">
                                        @forelse ($berita1 as $key => $item)
										<b class="is-visible">{{ $item->category->category_name}}</b>
										<b>BERITA</b>
                                        @empty
                                        <b class="is-visible">BERITA KATEGORI TERSEBUT BELUM ADA</b>
                                        <b>BERITA KATEGORI TERSEBUT BELUM ADA</b>
                                        @endforelse
									</span>
								</h2>
            </div>
            ...
            <div class="col-md-12 align-self-center order-1">
                ...
                <ul class="breadcrumb d-block text-center">
                    ...
                    <li class="active">BERITA {{ $pegawai["nama_skpd"] }}</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="col-lg-3 order-lg-2">
        <aside class="sidebar">
            ...
            @php
						$count = 0;
						@endphp
						@foreach ($category_list as $key => $kate)
										...
										<div class="col-lg-5">
											<div class="post-image">
												<a href="blog-post.html">
													<img src="{{ URL::to($item->picture) }}" class="img-fluid appear-animation" data-appear-animation="lightSpeedIn" data-appear-animation-delay="0" data-appear-animation-duration="1s"/>
												</a>
											</div>
										</div>
										<div class="col-lg-7">
											<div class="post-content">
												<h2 class="font-weight-semibold pt-4 pt-lg-0 text-5 line-height-4 mb-2"><a href="{{ url('berita/detail_berita/'.$item->title_slug) }}">{{ $item->title}}</a></h2>
									...
									<div class="row">
										<div class="col">
											<div class="post-meta">
												...
												<span><i class="far fa-folder"></i> <a href="{{ url('berita/berita_kategori/'.$item->category_id) }}">{{ $item->category->category_name}}</a></span>
												<span><i class="far fa-eye"></i>Pengunjung: {{ $item->hits}}</span>
												<span class="d-block d-sm-inline-block float-sm-right mt-3 mt-sm-0"><a href="{{ url('berita/detail_berita/'.$item->title_slug) }}" class="btn btn-xs btn-light text-1 text-uppercase">Baca Selengkapnya</a></span>
											</div>
										</div>
									</div>
								</article>
