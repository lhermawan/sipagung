                                    ...
                                    @foreach ($tenis as $key => $item)
                                        <tr>
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                               <a class="dropdown-item" href="{{ url('admin/pertandingan/soft_tenis/edit_pertandingan_soft_tenis_beregu/'.$item->id_pertandingan_soft_tenis) }}"><i class="fa fa-edit-o m-r-5"></i> Edit Data</a>
                                                <a class="dropdown-item" data-id="{{$item->id_pertandingan_soft_tenis}}" data-skor1="{{$item->skor_peserta1}}" data-skor2="{{$item->skor_peserta2}}"  data-status="{{$item->status_pertandingan}}"  class="btn add-btn" data-toggle="modal" data-target="#edit_skor" style="float: right;"><i class="fa fa-pencil-o m-r-5"></i> Edit Skor</a>
												<a class="dropdown-item" data-id="{{$item->id_pertandingan_soft_tenis}}" class="btn add-btn" data-toggle="modal" data-target="#hapus_tenis" style="float: right;"><i class="fa fa-trash-o m-r-5"></i> Hapus</a>
                                            </td>
                                                </div>
                                                </div>
										    </tr>
...
<div class="modal fade" role="dialog" id="imgmodal">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ route('saveSoftTenisBeregu') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<div class="modal fade" id="hapus_tenis" tabindex="-1" aria-labelledby="modalkdp" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalkdp">Hapus Jadwal Pertandingan {{$data2[0]->nama_jenis_cabor}} tersebut?</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </div>
            <div class="modal-body">
            <form action="{{ route('deleteJadwalSoftTenisSingle') }}" method="POST" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        {{ method_field('DELETE')}}
                    <div class="form-group">
                        <input type="text" class="form-control" id="id_pertandingan_soft_tenis" name="id_pertandingan_soft_tenis" readonly required>
                    </div>
...
<div class="modal fade" id="edit_skor" tabindex="-1" aria-labelledby="modalkdp" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
            <form action="{{ route('updateSkorSoftTenisBeregu') }}" method="POST" enctype="multipart/form-data">
                @csrf
                    
                <input type="hidden" readonly class="form-control" id="id_pertandingan_soft_tenis" name="id_pertandingan_soft_tenis" readonly>
...
var modal = $(this)
modal.find('.modal-body #id_pertandingan_soft_tenis').val(ID)
