								...
								<table class="table table-striped custom-table datatable">
                                    ...
                                    @foreach ($atletik as $key => $item)
										<tr>
                                            ...
                                            <td><span class="badge bg-success" style="font-size: 12px;">{{ $item->data_peserta->asal_kabkota }}</span></td>
                                            <td>
                                            <button type="button" class="btn btn-danger" href="#"  data-toggle="modal" data-target="#hapus_peserta" class="edit-icon" data-id="{{$item->id_pertandingan_atletik}}"  data-name="{{$item->data_peserta->asal_kabkota}}"  ><i class="fa fa-trash"></i> Delete</a></button>
                                            </td>
...
<div class="modal fade" id="add_peserta" tabindex="-1" aria-labelledby="modalkdp" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
            <form action="{{ route('saveTimAtletik') }}" method="POST" enctype="multipart/form-data">
                @csrf
                    
                <input type="hidden" readonly class="form-control" id="id_series_atletik" name="id_series_atletik" required>
 
                <div class="form-check">
                        <label for="formGroupExampleInput2">Silahkan Pilih Tim Peserta yang mengikuti {{ $data2[0]->jenis_cabor2->jenis_cabor->jenis_olahraga }} / {{ $data2[0]->jenis_cabor2->nama_jenis_cabor }} / {{ $data2[0]->nama_series_atletik }}: </label>
                        @foreach ($peserta1 as $key => $tim)
                    <div class="checkbox">
                        <label>
                        <input type="checkbox" class="form-check-input" name="id_data_peserta[]" value="{{$tim->id_data_peserta}}"><label>{{$tim->asal_kabkota}}</label><br/>
                        </label>
                     <div>
                @endforeach
                </div>
