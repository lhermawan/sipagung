                ...
                <div class="content container-fluid">
					...
					<div class="page-header">
						<div class="row align-items-center">
                            ...
                            <button type="button" data-idcabor2="{{$data2[0]->id_jenis_cabor2}}" class="btn add-btn" data-toggle="modal" data-target="#add_atletik" style="float: right;"><i class="bi bi-plus-lg"></i> Input Jadwal Pertandingan</button>
							</div>
						</div>
					</div>
							...
							<div class="table-responsive">
								<table class="table table-striped custom-table datatable">
                                    ...
                                    @foreach ($atletik as $key => $item)
                                        <tr>
											<td>{{ ++$key }}</td>
                                            <td>{{$item->nama_series_atletik }}</td>
                                            <td>
                                            <a type="button" class="btn btn-success"  href="{{ url('admin/pertandingan/atletik/lihat_peserta_tim/'.$item->id_series_atletik) }}"><i class="fa fa-users m-r-5"></i> Lihat Peserta</a>
                                            </td>
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                <a class="dropdown-item" href="{{ url('admin/pertandingan/atletik/edit_pertandingan_atletik/'.$item->id_series_atletik) }}"><i class="fa fa-edit-o m-r-5"></i> Edit Data</a>
                                                <a class="dropdown-item" href="{{ url('admin/pertandingan/atletik/edit_skor_pertandingan_atletik_tim/'.$item->id_series_atletik) }}"><i class="fa fa-edit-o m-r-5"></i> Edit Skor</a>
												<a class="dropdown-item" data-id="{{$item->id_series_atletik}}" class="btn add-btn" data-toggle="modal" data-target="#hapus_atletik" style="float: right;"><i class="fa fa-trash-o m-r-5"></i> Hapus</a>
                                            </td>
                                                </div>
                                                </div>
										    </tr>
						...
						<div class="col-md-12">
							<div class="table-responsive">
								<table class="table table-striped custom-table datatable">
									...
									<tbody>
                                    @foreach ($atletik2 as $key => $item)
                                        <tr>
											<td>{{ ++$key }}</td>
                                            <td>{{$item->nama_series_atletik }}</td>
                                            <td>
                                            <a type="button" class="btn btn-success"  href="{{ url('admin/pertandingan/atletik/lihat_peserta_atletik/'.$item->id_series_atletik) }}"><i class="fa fa-users m-r-5"></i> Lihat Peserta</a>
                                            </td>

                                            @if(empty($item->id_data_peserta_menang1 == ""))
                                            <td><span class="badge bg-success" style="font-size: 16px;">{{$item->peserta_menang1->asal_kabkota }} ( Skor {{$item->skor_pemenang1 }})</span></td>
                                            @else
                                            <td>TIDAK ADA</p></td>
                                            @endif

                                            @if(empty($item->id_data_peserta_menang2 == ""))
                                            <td><span class="badge bg-info" style="font-size: 16px;">{{$item->peserta_menang2->asal_kabkota }} ( Skor {{$item->skor_pemenang1 }})</span></td>
                                            @else
                                            <td>TIDAK ADA</p></td>
                                            @endif

                                            @if(empty($item->id_data_peserta_menang3 == ""))
                                            <td><span class="badge bg-danger" style="font-size: 16px;">{{$item->peserta_menang3->asal_kabkota }} (Skor {{$item->skor_pemenang3 }})</span></td>
                                            @else
                                            <td>TIDAK ADA</p></td>
                                            @endif
...
<div class="modal fade" role="dialog" id="imgmodal">
    ...
    <div class="modal fade" id="add_atletik" tabindex="-1" aria-labelledby="modalkdp" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalkdp">Tambah Jadwal Pertandingan Atletik {{$data2[0]->nama_jenis_cabor}}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </div>
            <div class="modal-body">
            <form action="{{ route('saveSeriesAtletik') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                        <label for="formGroupExampleInput2">Judul Pertandingan Atletik</label>
                        <input type="text" class="form-control" id="nama_series_atletik" name="nama_series_atletik" required>
                </div>
...
<div class="modal fade" id="hapus_atletik" tabindex="-1" aria-labelledby="modalkdp" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalkdp">Hapus Jadwal Pertandingan Atletik {{$data2[0]->nama_series_atletik}} tersebut?</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </div>
            <div class="modal-body">
            <form action="{{ route('deleteJadwalAtletik') }}" method="POST" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        {{ method_field('DELETE')}}
                    <div class="form-group">
                        <input type="hidden" class="form-control" id="id_series_atletik" name="id_series_atletik" readonly required>
                    </div>
...
<div class="modal fade" id="edit_skor" tabindex="-1" aria-labelledby="modalkdp" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
            <form action="{{ route('updateSkorPertandinganAtletik') }}" method="POST" enctype="multipart/form-data">
                @csrf
                    
                <input type="hidden" readonly class="form-control" id="id_series_atletik" name="id_series_atletik" readonly>
                ...
                <div class="form-row">
                    ...
                    <select name="id_peserta_menang_atletik1" class="form-control" id="id_peserta_menang_atletik1">
                        <option value="">-</option>             
                ...
                <div class="form-row">
                    ...
                    <select name="id_peserta_menang_atletik2" class="form-control" id="id_peserta_menang_atletik2">
                        <option value="">-</option>             
                        @foreach ($data3 as $peserta)
                                <option value="{{$peserta->atlit->id_atlit}}">{{$peserta->atlit->nama_atlit}} ({{$peserta->atlit->data_peserta->asal_kabkota}})</option>
                        @endforeach 
                ...
                <div class="form-row">
                    ...
                    <select name="id_peserta_menang_atletik3" class="form-control" id="id_peserta_menang_atletik3">
                        <option value="">-</option>             
                        @foreach ($data3 as $peserta)
                                <option value="{{$peserta->atlit->id_atlit}}">{{$peserta->atlit->nama_atlit}} ({{$peserta->atlit->data_peserta->asal_kabkota}})</option>
                        @endforeach 
...
$('#add_atletik').on('show.bs.modal', function (event) {
...
$('#hapus_atletik').on('show.bs.modal', function (event) {
