...
{{-- message --}}
    ...
    @endif
                ...
                <div class="content container-fluid">
                    ...
                    <div id="jadwal" class="tab-pane active">
                    <br>
							...
							<div class="table-responsive">
								<table class="table table-striped custom-table datatable">
                                    ...
                                    @foreach ($wushu as $key => $item)
                                        <tr>
											<td>{{ ++$key }}</td>
                                            <td>{{$item->nama_series }}</td>
                                            <td>{{$item->keterangan }}</td>
                                            <td>
                                            <a type="button" class="btn btn-success"  href="{{ url('admin/pertandingan/wushu/lihat_peserta_wushu/'.$item->id_pertandingan_wushu) }}"><i class="fa fa-users m-r-5"></i> Lihat Peserta</a>
                                            </td>
                                            ...
                                            <a class="dropdown-item" href="{{ url('admin/pertandingan/wushu/edit_pertandingan_wushu_team/'.$item->id_pertandingan_wushu) }}"><i class="fa fa-edit-o m-r-5"></i> Edit Data</a>
                                                <a class="dropdown-item" href="{{ url('admin/pertandingan/wushu/edit_skor_pertandingan_wushu/'.$item->id_pertandingan_wushu) }}"><i class="fa fa-edit-o m-r-5"></i> Edit Skor</a>
												<a class="dropdown-item" data-id="{{$item->id_pertandingan_wushu}}" class="btn add-btn" data-toggle="modal" data-target="#hapus_bmx" style="float: right;"><i class="fa fa-trash-o m-r-5"></i> Hapus</a>
                                            </td>
                                                </div>
                                                </div>
										    </tr>
								...
								<table class="table table-striped custom-table datatable">
                                    ...
                                    @foreach ($wushu2 as $key => $item)
                                        <tr>
                                            ...
                                            <td>{{$item->keterangan }}</td>
                                            <td><span class="badge bg-success" style="font-size: 16px;">{{$item->peserta_wushu_menang1->asal_kabkota }} ( Skor {{$item->skor_peserta1 }})</span></td>
                                            <td><span class="badge bg-info" style="font-size: 16px;">{{$item->peserta_wushu_menang2->asal_kabkota }} ( Skor {{$item->skor_peserta2 }})</span></td>
                                            @if(empty($item->team_menang3 == ""))
                                            <td><span class="badge bg-danger" style="font-size: 16px;">{{$item->peserta_wushu_menang3->asal_kabkota }} (Skor {{$item->skor_peserta3 }})</span></td>
                                            @else
                                            <td>TIDAK ADA</p>
                                            @endif
                                            <td>{{$item->status_pertandingan }}</td>
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                <a class="dropdown-item" href="{{ url('admin/pertandingan/wushu/edit_pertandingan_wushu_team/'.$item->id_pertandingan_wushu) }}"><i class="fa fa-edit-o m-r-5"></i> Edit Data</a>
                                                <a class="dropdown-item" href="{{ url('admin/pertandingan/wushu/edit_skor_pertandingan_wushu_team/'.$item->id_pertandingan_wushu) }}"><i class="fa fa-edit-o m-r-5"></i> Edit Skor</a>
												<a class="dropdown-item" data-id="{{$item->id_pertandingan_wushu}}" class="btn add-btn" data-toggle="modal" data-target="#hapus_bmx" style="float: right;"><i class="fa fa-trash-o m-r-5"></i> Hapus</a>
                                            </td>
                                                </div>
                                                </div>
...
<div class="modal fade" role="dialog" id="imgmodal">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ route('saveWushuTeam') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                        <label for="formGroupExampleInput2">Judul Series</label>
                        <input type="text" class="form-control" id="nama_series" name="nama_series" required>
                </div>
...
<div class="modal fade" id="hapus_bmx" tabindex="-1" aria-labelledby="modalkdp" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalkdp">Hapus Jadwal Pertandingan {{$data2[0]->nama_series}} tersebut?</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </div>
            <div class="modal-body">
...
<div class="modal fade" id="edit_skor" tabindex="-1" aria-labelledby="modalkdp" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
            <form action="{{ route('updateSkorPertandinganPentaqueSingle') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-row">
                    ...
                    <input type="text" class="form-control" id="skor_peserta3" name="skor_peserta3" required>
                    </div>
                </div>
