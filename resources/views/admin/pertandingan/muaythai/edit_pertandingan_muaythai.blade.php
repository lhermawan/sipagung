...
<div class="page-wrapper">
                <div class="content container-fluid">
					...
					<div class="page-header">
						<div class="row">
							<div class="col">
								<h3 class="page-title">Edit Pertandingan Muaythai</h3>
								<ul class="breadcrumb">
									<li class="breadcrumb-item"><a href="index.html"></a>Dashboard</li>
									<li class="breadcrumb-item active">Pertandingan Muaythai</li>
								</ul>
							</div>
						</div>
					</div>
						...
						<div class="col-lg-12">
							<div class="card">
								<div class="card-header">
									<h4 class="card-title mb-0">Edit Pertandingan Muaythai</h4>
								</div>
								<div class="card-body">
                                <form class="form form-horizontal" action="{{ route('updatePertandinganMuaythai') }}" method="POST" enctype="multipart/form-data">
                                        @csrf
   
                                        <div class="form-group row">
                                        <label class="col-sm-2 col-form-label">Peserta 1</label>
                                            <div class="col-sm-10">
                                                <input class="form-control @error('id_pertandingan_muaythai') is-invalid @enderror" readonly name="id_pertandingan_muaythai" value="{{$data[0]->id_pertandingan_muaythai}}" type="hidden" id="id_pertandingan_muaythai" required>
                                                ...
                                                <select name="id_atlit1" class="form-control" id="id_atlit1">
                                                <option value="{{ $data[0]->id_atlit1}}" selected>--Pilihan Sebelumnya ( {{ $data[0]->atlit1->nama_atlit}} / {{ $data[0]->atlit1->data_peserta->asal_kabkota}})--</option>
                                                @foreach ($data3 as $peser => $item)
                                                        <option id="id_atlit1" value="{{$item->id_atlit}}">{{$item->nama_atlit}} ( {{ $item->asal_kabkota}} )</option>
                                                @endforeach 
                                                </select>
                                                @error('id_atlit1')
                                        ...
                                        <label class="col-sm-2 col-form-label">Peserta 2</label>
                                            <div class="col-sm-10">
                                                <select name="id_atlit2" class="form-control" id="id_atlit2">
                                                <option value="{{ $data[0]->id_atlit2}}" selected>--Pilihan Sebelumnya ( {{ $data[0]->atlit2->nama_atlit}} / {{ $data[0]->atlit2->data_peserta->asal_kabkota}})--</option>
                                                @foreach ($data3 as $peser => $item)
                                                        <option id="id_atlit2" value="{{$item->id_atlit}}">{{$item->nama_atlit}} ( {{ $item->asal_kabkota}} )</option>
                                                @endforeach 
                                                </select>
                                            ...
                                            <div class="col-sm-10">
                                                        <input type="file" class="form-control"
                                                            ...
                                                            <div class="d-flex mr-3 rounded-circle thumb-md">
                                                                <img class="d-flex mr-3 rounded-circle thumb-md" width="100" height="100" src="{{ URL::to('/images/muaythai/'. $data[0]->avatar) }}">
                                                            </div>
