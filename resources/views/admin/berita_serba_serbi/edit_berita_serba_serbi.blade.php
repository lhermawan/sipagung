					...
					<div class="row">
						<div class="col-lg-12">
							<div class="card">
								...
								<div class="card-body">
                                <form action="{{ route('admin/berita_serba_serbi/update_berita_serba_serbi') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                    <div class="form-group row">
                                        ...
                                        <label class="col-sm-2 col-form-label">Kategori Berita</label>
                                            <div class="col-sm-10">
                                            <select class="form-control @error('id_kategori_berita') is-invalid @enderror"  value="{{ $data[0]->id_kategori_berita}}" name="id_kategori_berita" id="id_kategori_berita" required>
                                            <option value="{{ $data[0]->id_kategori_berita}}" selected>--Pilihan Sebelumnya ({{ $data[0]->kategori_berita_serba_serbi->nama_kategori}})</option>
                                            @foreach ($kategori as $peser => $item)
                                                        @if (old('id') == $peser)
                                                        <option id="id_kategori_berita" value="{{$item->id_kategori_berita}}" >{{ $item->nama_kategori}}</option>
                                                        @else
                                                        <option id="id_kategori_berita" value="{{$item->id_kategori_berita}}">{{ $item->nama_kategori}}</option>
                                                    @endif
                                                @endforeach 
                                            ...
                                            <div class="col-sm-10">
                                                        <input type="file" class="form-control"
                                                            ...
                                                            <div class="d-flex mr-3 rounded-circle thumb-md">
                                                                <img class="d-flex mr-3 rounded-circle thumb-md" width="100" height="100" src="{{ URL::to('/images/berita_serba_serbi/'. $data[0]->avatar) }}">
                                                            </div>
