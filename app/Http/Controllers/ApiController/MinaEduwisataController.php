...
class MinaEduwisataController extends Controller
{
    ...
    public function post_eduwisata(Request $request)
    {
        $token = Auth::user() != '' ?  Auth::user()->token : $this->secret_key;
        $check = $this->authorization($token);
        if ($check >0) {
            try {
                    $data = new \App\Models\MinaEduwisata();
                    ...
                    if ($data->picture && $data->picture->isValid()) {
                        ...
                        $request->picture->move(public_path('images/foto_minawisata'), $file_name);
                        $path = "images/foto_minawisata/$file_name";
                        $data->picture = $path;
                    }else{
    ...
    public function update_eduwisata(Request $request)
    {
        ...
        if ($check > 0) {
            try {
                ...
                MinaEduwisata::where('post_id',$request["post_id"])->update($input);
    ...
    public function delete_eduwisata(Request $request)
    {
        ...
        if ($check > 0) {
            try {
                ...
                MinaEduwisata::where('post_id',$request["post_id"])->delete();
