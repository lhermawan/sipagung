...
use App\Models\PaketMinaEduwisata;
...
class PaketMinaEduwisataController extends Controller
{
    /**
    ...
    public function post_kampung_nila_paketmina(Request $request)
    {
        $token = Auth::user() != '' ?  Auth::user()->token : $this->secret_key;
        $check = $this->authorization($token);
        if ($check >0) {
            try {
                    $data = new \App\Models\PaketMinaEduwisata();
    ...
    public function update_paketmina_kampungnila(Request $request)
    {
        // dd('a');die;
        $token = Auth::user() != '' ?  Auth::user()->token : $this->secret_key;
        $check = $this->authorization($token);
        if ($check > 0) {
            try {
                ...
                PaketMinaEduwisata::where('post_id',$request["post_id"])->update($input);
    ...
    public function delete_paketmina_kampungnila(Request $request)
    {
        // dd('a');die;
        ...
        if ($check > 0) {
            try {
                ...
                PaketMinaEduwisata::where('post_id',$request["post_id"])->delete();
